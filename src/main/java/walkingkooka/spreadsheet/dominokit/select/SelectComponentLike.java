...
abstract class SelectComponentLike<T> implements FormValueComponent<HTMLFieldSetElement, T, SelectComponent<T>>,
    ...
    @Override
    public final SelectComponent<T> addFocusInListener(final EventListener listener) {
        return this.addEventListener(
            EventType.focusin,
            listener
        );
    ...
    @Override
    public final SelectComponent<T> addFocusOutListener(final EventListener listener) {
        return this.addEventListener(
            EventType.focusout,
            listener
        );
    ...
    final ValueWatchers<T> valueWatchers = ValueWatchers.empty();
}
